...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        }  # Putting form data into a dictionary
        order_form = OrderForm(form_data)
        if order_form.is_valid():
            order = order_form.save(commit=False) # Commit = false prevents multiple save events happening on database
            pid = request.POST.get('client_secret').split('_secret')[0]
            ...
            for item_id, item_data in bag.items():
                try:
                    product = Product.objects.get(id=item_id)  # Get item from bag
                    if isinstance(item_data, int):  # If value is an integer then know working with product without sizes so quantity will just be the item data
                        ...
                        for size, quantity in item_data['items_by_size'].items():  # Otherwise iterate through each size to create line item
                            order_line_item = OrderLineItem(
                                order=order,
                                product=product,
                                quantity=quantity,
                            ...
                            order_line_item.save()
                except Product.DoesNotExist:  # If item weren't found (shouldnt happen) will delete order and return to shopping bag page
            ...
            request.session['save_info'] = 'save-info' in request.POST  # Whether user wanted to save their profile info to the session
        ...
        current_bag = bag_contents(request)  # i.e. making sure not to overwrite existing bag variable
        total = current_bag['grand_total']
        stripe_total = round(total*100)  # As Stripe needs to know amount to charge as an integer
        ...
        if request.user.is_authenticated:
            try:
                ...
                order_form = OrderForm(initial={ # Use initial parameter to prefill
...
def checkout_success(request, order_number):
    ...
    """
    save_info = request.session.get('save_info')  # Did user want to save info - get from session just like shopping bag
